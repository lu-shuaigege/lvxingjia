                    ...
                    <div class="popupcon-item">
                        ...
                        <div class="twoclass">
                            <div
                                ...
                                "
                                v-for="(item, indexa) in get_price.single"
                                :key="indexa"
                            >
                                <div>套餐{{ indexa + 1 }}</div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        twoclassfn: function(y) {
            this.twoclass = y;
            if (y == 2) {
                let num = this.get_price.single.length + 1;
        ...
        ticketbookingfn: function() {
            if (this.twoclass == 1) {
                ...
                let goodsDeatilsInfo = {
                    ...
                    order_type: 1, //类型 1门票 2酒店 3线路
                    name: this.setmeal_name, //详情名字
        ...
        // 请求景点门票详情
        getscenic_spotidfn: function() {
            let _this = this;
            this.$api.scenic_spot.show(_this.detailid).then(res => {
                _this.datas = res;
                _this.get_detail = res.get_detail;
                _this.get_price = res.get_price;
                _this.twoclassitem = res.get_price.single[0].id; //取第一个id
                _this.price_money = res.get_price.single[0].price; //取第一个价格
                _this.setmeal_name = res.get_price.single[0].name; //取第一个价格
                _this.goodsUnitPrice = res.get_price.single[0].price; //取第一个价格
                _this.buy_limit = res.get_price.single[0].limit_num; //取第一个最少购买数量  当前商品的单价
                _this.store = res.get_price.single[0].store; //取第一个的库存  当前商品的库存
